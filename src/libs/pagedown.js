import * as rangee from '../services/services';
...
var defaultsStrings = {
  ...
  inlinelinkbefore: "[",
  inlinelinkexample: "enter link description here",
  inlinelinkafter: "](https://example.com)",

  // inlineimage: "Inlineimage <img> Ctrl/Cmd+I",
  inlineimagebefore: "![",
  inlineimageexample: "enter image description here",
  inlineimageafter: "](https://via.placeholder.com/100)",
...
function TextareaState(input) {
  ...
  this.init = function () {
    ...
    this.text = rangee.getTextContent(inputArea);
  };
  ...
  this.setInputAreaSelection = function () {
    inputArea.focus();
    rangee.setSelection(inputArea, stateObj.start, stateObj.end)
    // inputArea.setSelection(stateObj.start, stateObj.end);
  };

  this.setInputAreaSelectionStartEnd = function () {
    stateObj.start = rangee.getSelectedOffset(inputArea)[0];
    stateObj.end = rangee.getSelectedOffset(inputArea)[1];
  };

  // Restore this state into the input area.
  this.restore = function () {

    if (stateObj.text !== undefined && stateObj.text != rangee.getTextContent(inputArea)) {
      rangee.setTextContent(inputArea, stateObj.text);
    }
...
function UIManager(input, commandManager) {
  ...
  function makeSpritedButtonRow() {
    ...
    buttons.math = bindCommand("doMath");
    buttons.link = bindCommand("doLinkInline");
    buttons.quote = bindCommand("doBlockquote");
    buttons.code = bindCommand("doCode");
    buttons.image = bindCommand("doImageInline");
...
commandProto.doStrikethrough = function (chunk) {
  ...
  // Remove stars if we have to since the button acts as a toggle.
  if (prevStars == 2) {
    chunk.before = chunk.before.replace(re("[~]{2}$", ""), "");
    chunk.after = chunk.after.replace(re("^[~]{2}", ""), "");
  } else {
...
commandProto.doMath = function (chunk) {
  ...
  // Remove stars if we have to since the button acts as a toggle.
  if (prevStars == 1) {
    chunk.before = chunk.before.replace(re("[\$]", ""), "");
    chunk.after = chunk.after.replace(re("^[\$]", ""), "");
  } else {
...
commandProto.doLinkInline = function (chunk) {
  ...
  if (!chunk.selection) {
    chunk.selection = this.getString("inlinelinkexample");
  }

  chunk.before = chunk.before + this.getString("inlinelinkbefore");
  chunk.after = this.getString("inlinelinkafter") + chunk.after;
...
commandProto.doImageInline = function (chunk) {
  ...
  if (!chunk.selection) {
    chunk.selection = this.getString("inlineimageexample");
  }

  chunk.before = chunk.before + this.getString("inlineimagebefore");
  chunk.after = this.getString("inlineimageafter") + chunk.after;
...
commandProto.doHeading = function (chunk) {
  ...
  // We make a level 1 header if there is no current header.
  // If there is a header level, we substract one from the header level.
  // If it's already a level 1 header, it's removed.
  var headerLevelToCreate = headerLevel === 6 ? 0 : headerLevel + 1;
